...
// temporary user ID --> fix in sprint 3
...
const PostInterventionRating: React.FC = () => { 
    ...
    // hardcoded user ID --> fix in sprint 3
    const userId = TEMP_USER_ID; 

    // Function for saving the post baseline data to the Express server (MongoDB)
    const handleSaveRating = async () => {
        ...
        const dataToSave = {
            ...state,
            // keep different than pre-intervention
            type: 'baseline_post_intervention', 
            userId: userId, 
        };

        // updated endpoint for post-intervention -- match to deployment URL
        const API_URL = 'http://192.168.1.8:3000/api/ratings/postline'; 

        try {
            const response = await axios.post(API_URL, dataToSave);
            
            Alert.alert("Success", response.data.msg || "Final assessment data saved! Study complete.");
            
            // Navigate to the Results or Dashboard after completion
    ...
    // Helper to get text for the slider label based on value
    ...
    return (
        <View style={[styles.mainView, { paddingTop: insets.top, backgroundColor: theme.colors.background }]}>
            <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled">
                <Text variant="headlineMedium" style={styles.headline}>Final Self-Rating</Text>
                <Text variant="bodyMedium" style={styles.subTitle}>
                    Please provide your current well-being and stress scores at the conclusion of the study.
                </Text>
                ...
                <TextInput
                    ...
                    mode="outlined" // Provides the standard Paper outline style
                    placeholder="e.g., 'I feel much better after the intervention.'"
                    multiline
                    numberOfLines={4}
                ...
                >
                    {isLoading ? 'Saving...' : 'Complete Study & Submit Results'}
                </Button>
            </ScrollView>
        </View>
    );
...
export default PostInterventionRating;