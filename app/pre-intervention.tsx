...
const savePreRatingToMongo = async (userId: string, data: ScoreData): Promise<void> => {
    const MONGODB_API_URL = "https://mockapi.example.com/api/v1/ratings";
    
    console.log(`Attempting to save PRE-RATING data to (mock) endpoint: ${MONGODB_API_URL}`);
    ...
    try {
        await fetch(MONGODB_API_URL, {
            ...
            body: JSON.stringify({
                ...
                type: 'pre-intervention', // Distinct type for Pre
                ...data,
            }),
        });
        ...
        throw new Error("Failed to save pre-intervention data to the mock MongoDB endpoint.");
    }
};
...
const PreRatingScreen: React.FC = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await savePreRatingToMongo(userId, scores);
            
            setStatusMessage('Baseline saved! Proceeding to Intervention.');
            console.log("Pre-Intervention Scores submitted successfully:", scores);
